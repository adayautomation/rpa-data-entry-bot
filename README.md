...
1. Reads member data from an Excel file (`datos_comuneros.xlsx`)
2. Navigates the Affinity platform UI using screenshot-based detection
3. Fills in personal details, tax address, province, and municipality fields
4. Handles dropdown menus with dynamic scroll-and-detect logic
5. Confirms completion and moves on

This was built to automate a repetitive manual task at a water management cooperative in Tenerife, Spain, reducing data entry time from ~8 minutes per record to under 60 seconds.
...
├── datos_comuneros.xlsx             # Input data (fictional sample included)
...
The bot uses image recognition to find buttons and fields on screen. Capture your own screenshots of the Affinity interface and place them in the `assets/` folder:
...
- `campo_provincia.png`, `opcion_tenerife.png` — Province dropdown
- `campo_municipio.png`, `zona_lista_municipios.png`, `opcion_laguna.png` — Municipality scroll
...
You have 5 seconds after launch to switch focus to the Affinity window.
...
The target platform (Affinity) is a desktop-style web app with no stable DOM selectors. Visual detection via screenshots was more reliable than trying to target elements with CSS selectors.
...
- Requires the Affinity window to be visible and unobstructed during execution
